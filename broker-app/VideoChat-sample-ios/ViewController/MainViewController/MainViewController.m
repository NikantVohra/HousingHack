...
- (void)viewDidLoad
{
    ...
    self.imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.view.center.y - 20, 60, 60)];
    //
...
- (void)call:(id)sender{
    ...
    if(callButton.tag == 101){
        ...
        if(self.videoChat == nil){
            ...
            self.videoChat.viewToRenderOwnVideoStream = videoView;
            //self.videoChat.useBackCamera = 1;
        }
...
- (void)initializePubNubClient {
    ...
    [[PNObservationCenter defaultCenter] addMessageReceiveObserver:self
                                                         withBlock:^(PNMessage *message) {
                                                             if([message.channel.name isEqualToString:@"HousingPosition"]) {
                                                                 NSDictionary *messageDict = (NSDictionary *)message.message;
                                                                 NSString *direction = [messageDict objectForKey:@"position"];
                                                                 [self.imageView removeFromSuperview];
                                                                 if([direction isEqualToString:@"left"]) {
                                                                     self.imageView.frame = CGRectMake(10, self.view.center.y - 30, 60, 60);
                                                                     self.imageView.image = [UIImage imageNamed:@"leftarrow"];
                                                                     [self.arController.displayView addSubview:self.imageView];
                                                                     AVSpeechSynthesizer *synthesizer = [[AVSpeechSynthesizer alloc]init];
                                                                     AVSpeechUtterance *utterance = [AVSpeechUtterance speechUtteranceWithString:@"Turn Left"];
                                                                     [utterance setRate:0.1f];
                                                                     [synthesizer speakUtterance:utterance];
                                                                 }
                                                                 if([direction isEqualToString:@"right"]) {
                                                                     self.imageView.frame = CGRectMake(self.view.bounds.size.width - 80, self.view.center.y - 30, 60, 60);
                                                                     self.imageView.image = [UIImage imageNamed:@"rightarrow"];
                                                                     [self.arController.displayView addSubview:self.imageView];
                                                                     AVSpeechSynthesizer *synthesizer = [[AVSpeechSynthesizer alloc]init];
                                                                     AVSpeechUtterance *utterance = [AVSpeechUtterance speechUtteranceWithString:@"Turn Right"];
                                                                     [utterance setRate:0.1f];
                                                                     [synthesizer speakUtterance:utterance];
                                                                 }
                                                                 
                                                                 if([direction isEqualToString:@"up"]) {
                                                                     self.imageView.frame = CGRectMake(self.view.center.x, 30.0f , 60, 60);
                                                                     self.imageView.image = [UIImage imageNamed:@"uparrow"];
                                                                     [self.arController.displayView addSubview:self.imageView];
                                                                     AVSpeechSynthesizer *synthesizer = [[AVSpeechSynthesizer alloc]init];
                                                                     AVSpeechUtterance *utterance = [AVSpeechUtterance speechUtteranceWithString:@"Move Forward"];
                                                                     [utterance setRate:0.1f];
                                                                     [synthesizer speakUtterance:utterance];
                                                                 }
                                                                 
                                                                 if([direction isEqualToString:@"down"]) {
                                                                     self.imageView.frame = CGRectMake(self.view.center.x, self.view.bounds.size.height - 80, 60, 60);
                                                                     self.imageView.image = [UIImage imageNamed:@"downarrow"];
                                                                     [self.arController.displayView addSubview:self.imageView];
                                                                     AVSpeechSynthesizer *synthesizer = [[AVSpeechSynthesizer alloc]init];
                                                                     AVSpeechUtterance *utterance = [AVSpeechUtterance speechUtteranceWithString:@"Move Backward"];
                                                                     [utterance setRate:0.1f];
                                                                     [synthesizer speakUtterance:utterance];
                                                                 }
                                                             }
...
-(void)connectToPubnub {
    [PubNub connectWithSuccessBlock:^(NSString *origin) {
        ...
        PNChannel *channel = [PNChannel channelWithName:@"HousingPosition"
